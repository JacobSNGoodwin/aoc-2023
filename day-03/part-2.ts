...
const getSurroundingSymbols = (
  ...
  const symbols = new Set<[number, number]>();
  ...
  for (let i = numberInfo.startIndex - 1; i <= numberInfo.endIndex + 1; i++) {
    ...
    // });
    if (top === '*') {
      symbols.add([numberInfo.row - 1, i]);
    }
  ...
  // });
  if (leftSide === '*') {
    symbols.add([numberInfo.row, numberInfo.startIndex - 1]);
  }

  if (rightSide === '*') {
    symbols.add([numberInfo.row, numberInfo.endIndex + 1]);
  }
  return symbols;
};

type NumberSymbols = NumberInfo & {
  symbols: Set<[number, number]>;
};
...
// group by symbol coordinates

type SymbolRecord = Record<string, number[]>;

const groupedBySymbol = numbersWithSymbols.reduce<SymbolRecord>(
  (current, numberSymbols) => {
    numberSymbols.symbols.forEach(([row, col]) => {
      const key = `${row}:${col}`;
      if (current[key]) {
        current[key].push(numberSymbols.value);
      } else {
        current[key] = [numberSymbols.value];
      }
    });
    return current;
  },
...
// console.debug('groupedBySymbol', groupedBySymbol);

const sum = Object.values(groupedBySymbol).reduce((total, vals) => {
  if (vals.length === 2) {
...
console.debug('part 2 result', sum);
